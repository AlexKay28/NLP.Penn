...
public class PrintFalsePositives {
	...
	public static void main(String[] args) {
		try {
			...
			PrintWriter pwr = new PrintWriter(new FileWriter("falsePositives.txt"));
			for (String txtFile : txtFiles) {
				BufferedReader br1 = new BufferedReader(new FileReader("../trn_txt/"+txtFile));
				...
				while(br1.ready()){
					...
					for (String contestant : contestants) {
						...
						String interaction = findInteraction(contestant,1.0);
						//Sid.log(interaction+"\n");
						if(interaction!=null){
							String output=(interaction.split("\\|\\|")[2]+"||"+interaction.split("\\|\\|")[3]+"||"+interaction.split("\\|\\|")[4]);
							String[] outputTokens=output.split("\\|\\|");
							ArrayList<String> goldStandard = new ArrayList<String>();
							BufferedReader br11 = new BufferedReader(new FileReader(
									"../trn_rel/" + txtFile.substring(0,txtFile.length()-3)+"rel"));
							while (br11.ready()) {
								String line1 = br11.readLine().trim();
								goldStandard.add(line1);
								//Sid.log(line);
							}
							br11.close();
							boolean found=false;
							...
							if(!found){
								String printIt=txtFile+"||"+lineNumber+"||"+line+"||"+contestant+"||"+interaction;
								Sid.log("fp:\t"+printIt.replaceAll("\\|\\|", "\t"));
								pwr.println(printIt.replaceAll("\\|\\|", "\t"));
							}
						}
						if(linePatterns.containsKey(contestant)&&!patterns.contains(linePatterns.get(contestant))){
	...
	private static String findInteraction(String contestant, double threshold) {
		int ent1=-1,ent2=-1;
		...
		*/
		...
		if(results.size()>0){
			...
			if(posLabels.contains("PIP"))
				...
				if(max>0&&countI==max)
					return max+"||"+as.get(0).getKey().split("\\|\\|")[1]+"||"+sortedConcepts.get(ent1)+"||r=\""+"TrIP"+"\"||"+sortedConcepts.get(ent2);
				if(max>0&&countW==max)
					return max+"||"+as.get(0).getKey().split("\\|\\|")[1]+"||"+sortedConcepts.get(ent1)+"||r=\""+"TrWP"+"\"||"+sortedConcepts.get(ent2);
				if(max>0&&countC==max)
					return max+"||"+as.get(0).getKey().split("\\|\\|")[1]+"||"+sortedConcepts.get(ent1)+"||r=\""+"TrCP"+"\"||"+sortedConcepts.get(ent2);
				if(max>0&&countA==max)
					return max+"||"+as.get(0).getKey().split("\\|\\|")[1]+"||"+sortedConcepts.get(ent1)+"||r=\""+"TrAP"+"\"||"+sortedConcepts.get(ent2);
				if(max>0&&countNA==max)
					return max+"||"+as.get(0).getKey().split("\\|\\|")[1]+"||"+sortedConcepts.get(ent1)+"||r=\""+"TrNAP"+"\"||"+sortedConcepts.get(ent2);
				...
				if(max>0&&countR==max)
					return max+"||"+as.get(0).getKey().split("\\|\\|")[1]+"||"+sortedConcepts.get(ent1)+"||r=\""+"TeRP"+"\"||"+sortedConcepts.get(ent2);
				if(max>0&&countC==max)
					return max+"||"+as.get(0).getKey().split("\\|\\|")[1]+"||"+sortedConcepts.get(ent1)+"||r=\""+"TeCP"+"\"||"+sortedConcepts.get(ent2);
			}
		}
